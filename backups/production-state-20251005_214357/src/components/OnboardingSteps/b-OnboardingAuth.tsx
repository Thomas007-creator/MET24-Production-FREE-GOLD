...
import { connectToGrok, saveXAuthToDatabase } from '../../config/xOAuthConfig';
...
const OnboardingAuth: React.FC<OnboardingAuthProps> = ({ onNext }) => {
  ...
  const handleXAuth = async () => {
    ...
    try {
      ...
      // Grok-4's X OAuth implementation
      const accessToken = await connectToGrok();

      // Simulate X auth success (in real app, this would be actual OAuth)
      const mockProfile = {
        given_name: 'X',
        family_name: 'User',
        email: 'user@x.com',
      ...
      const userId = 'user_x_123'; // In real app, this would be the actual user ID
      ...
      if (accessToken) {
        await saveXAuthToDatabase(userId, accessToken, accessToken);
      }

      logger.info('X auth completed successfully');
      onNext();
    } catch (error) {
      logger.error('Error in X auth:', { error: error instanceof Error ? error.message : String(error) });
      // Still proceed even if database fails
      onNext();
    }
  };
