...
class ActiveImaginationChatLLMService {
  ...
  async recognizePatterns(
    ...
    try {
      ...
      const patternAnalysis = await chatLLMService.processPatternRecognition(
      ...
      return this.parseIdentifiedPatterns(patternAnalysis.result || patternPrompt, mbtiType, timeframe);
    ...
    try {
      ...
      await database.write(async () => {
        await chatMessages.create((message: any) => {
          message.user_id = (sessionMessages[0] as any)?.user_id;
          message.session_id = sessionId;
          ...
          message.created_at = Date.now();
          message.updated_at = Date.now();
          message.created_by = (sessionMessages[0] as any)?.user_id;
        });
      });
      ...
      const guidance = await chatLLMService.creativeImagination(
        guidancePrompt,
        sessionMessages[0]?.user_id || 'anonymous',
        mbtiType,
        {
          sessionId,
      ...
      await database.write(async () => {
        await chatMessages.create((message: any) => {
          message.user_id = (sessionMessages[0] as any)?.user_id;
          message.session_id = sessionId;
          message.message = guidance.response || '';
          message.is_user = false;
          message.message_type = 'guidance';
          message.context_type = 'imagination';
          message.timestamp = Date.now();
  ...
  async completeImaginationSession(
    ...
    try {
      ...
      const insightAnalysis = await chatLLMService.processJournalAnalysis(
        insightPrompt,
        5, // neutral mood rating for session analysis
        new Date().toISOString().split('T')[0],
        mbtiType
      );
      
      // Parse and return insights
      return this.parseImaginationInsights(insightAnalysis.result || insightPrompt, sessionId, mbtiType);
