...
type ChatLLMFeature = 
  ...
  | 'active_imagination'; // ✨ New: Active Imagination feature
...
class ChatLLMServiceManager {
  ...
  /**
   * 🧘‍♀️ Active Imagination - MBTI-gebaseerde creatieve visualisatie
   */
  async activeImagination(
    userInput: string, 
    userId: string, 
    mbtiType: string,
    step: number = 1,
  ...
  private async createInitialAuditEvent(requestId: string, traceId: string, request: ChatLLMRequest): Promise<AuditEvent> {
    ...
    return await auditEventServiceV14.createAuditEvent({
      ...
      dataSensitivityLevel: request.input.sensitivityLevel || 'PUBLIC',
