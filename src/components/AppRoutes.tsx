...
const JournalingPage = React.lazy(() => import('./EnhancedJournalingPage'));
      ...
      <Routes>
        ...
        {onboardingCompleted && (
          <>
            ...
            {/* Unified Upgrade page - handles both Gold and Platinum */}
            <Route path="/upgrade-platinum" element={
              <React.Suspense fallback={<div className="min-h-screen flex items-center justify-center text-white">Loading...</div>}>
                <UpgradePage 
                  userName="Gebruiker"
                  mbtiType="INTP"
                  onSelectSubscription={(planId: string, tier?: string) => {
                    logger.info('Plan selected', { planId, tier });
                    ...
                    if (tier?.includes('platinum')) {
                      ...
                      alert(`🎉 Upgrade naar Gold ${planId} succesvol! Veel plezier!`);
                      navigate('/');
                    }
                  }}
                  onCancel={() => navigate('/')}
