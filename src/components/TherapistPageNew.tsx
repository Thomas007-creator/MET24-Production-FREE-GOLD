...
import { Card, CardBody, CardHeader, Button, Chip, Progress, Input, Textarea, Spinner } from '@nextui-org/react';
import { 
  ArrowLeft, 
  Home, 
  Brain, 
...
const TherapistPageNew: React.FC = () => {
  const { userData } = useAppStore();
  const [currentView, setCurrentView] = useState<'overview' | 'ai-therapy' | 'find-therapist' | 'sessions' | 'coaches'>('overview');
  ...
  const userLocation = userData?.location || 'Nederland';
  ...
  useEffect(() => {
    const loadTherapistData = async () => {
      try {
        setIsLoading(true);
        logger.info('🔄 Loading therapist ecosystem data...');
        ...
        const status = await therapistService.getEcosystemStatus();
        setEcosystemStatus(status);

        // Mock therapy sessions (these could be loaded from database in future)
        const mockSessions: TherapySession[] = [
          {
            id: '1',
            type: 'ai',
            title: `${mbtiType} Emotionele Expressie Sessie`,
            description: `AI-gestuurde sessie voor het ontwikkelen van emotionele expressie en creativiteit specifiek voor ${mbtiType} types.`,
            duration: 30,
            mbtiFocus: [mbtiType],
            difficulty: 'beginner',
            category: 'therapy'
        ...
        logger.info('✅ Therapist ecosystem data loaded successfully');
      } catch (error) {
        logger.error('❌ Failed to load therapist ecosystem data:', { error });
      } finally {
        setIsLoading(false);
      }
    };
    ...
    loadTherapistData();
  }, [mbtiType, userLocation, userData?.userId]);
  ...
  const getMBTIRecommendations = (mbti: string) => {
    const recommendations = regionalDataService.getMBTIMatchingRecommendations(mbti);
    return recommendations;
  };

  const handleBookTherapist = (therapist: Therapist) => {
    if (therapist.externalBookingUrl) {
      window.open(therapist.externalBookingUrl, '_blank');
    } else {
      logger.info('Booking therapist', { therapistName: therapist.name });
      // Could implement internal booking system here
    }
  };

  const handleBookCoach = (coach: ZoomSpaceCoach) => {
    logger.info('Booking coach', { coachName: coach.name });
    // Implement ZoomSpace booking system
    // This would integrate with the ZoomSpace booking system
  };
          ...
          <div>
            ...
            {ecosystemStatus && (
              <div className='flex items-center gap-4 mt-2 text-sm text-gray-400'>
                <span>🏥 {ecosystemStatus.therapists} therapeuten</span>
                <span>🎯 {ecosystemStatus.coaches} coaches</span>
                <span>📊 {ecosystemStatus.wellnessAssessments} wellness assessments</span>
              </div>
            )}
          </div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
              ...
              <Card className='glass border border-white/10 hover:bg-white/10 transition-all cursor-pointer' onClick={() => setCurrentView('find-therapist')}>
                <CardBody className='p-6 text-center'>
                  ...
                  <h3 className='font-semibold text-white mb-2'>Therapeuten</h3>
                  <p className='text-sm text-gray-300'>Professionele begeleiding</p>
                </CardBody>
              </Card>
              <Card className='glass border border-white/10 hover:bg-white/10 transition-all cursor-pointer' onClick={() => setCurrentView('coaches')}>
                <CardBody className='p-6 text-center'>
                  <div className='text-4xl mb-3'>🎯</div>
                  <h3 className='font-semibold text-white mb-2'>Coaches</h3>
                  <p className='text-sm text-gray-300'>Gratis online begeleiding</p>
                </CardBody>
              </Card>
...
export default TherapistPageNew;
