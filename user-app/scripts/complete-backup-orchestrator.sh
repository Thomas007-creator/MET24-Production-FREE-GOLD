...
LATEST_PRODUCTION=$(find backups -name "production-state-*" -type d | sort | tail -1)
LATEST_DEPLOYMENT=$(find backups -name "digitalocean-deployment-*" -type d | sort | tail -1)

if [ -d "$LATEST_PRODUCTION" ]; then
    cp -r "$LATEST_PRODUCTION" "$MASTER_BACKUP_DIR/production-state/"
else
    echo "⚠️ Warning: No production backup found"
fi

if [ -d "$LATEST_DEPLOYMENT" ]; then
    cp -r "$LATEST_DEPLOYMENT" "$MASTER_BACKUP_DIR/digitalocean-deployment/"
else
    echo "⚠️ Warning: No deployment backup found"
fi
